  ...
  return <div className="min-h-screen bg-background pb-24">
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-8">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => navigate("/capture?mode=exterior")} className="card-elevated group hover:shadow-lg transition-all interactive-scale">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            {mockSessions.slice(0, 2).map(session => <button key={session.id} onClick={() => navigate(`/gallery/${session.id}`)} className="w-full card-elevated hover:shadow-lg transition-all interactive-scale">
                <div className="flex items-center gap-3 p-3">
                  <div className="w-20 h-20 bg-muted rounded-xl overflow-hidden flex-shrink-0">
                    {session.images[0]?.after ? <img src={session.images[0].after} alt={session.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center">
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <Badge variant="secondary" className={session.images[0]?.status === "processed" ? "bg-success/10 text-success border-success/20 mt-1" : "bg-warning/10 text-warning border-warning/20 mt-1"}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-card border-t border-border z-10">
        <div className="max-w-md mx-auto flex items-center justify-around py-3">
          <Button variant="ghost" size="sm" onClick={() => navigate("/home")} className="flex-col h-auto py-2">
